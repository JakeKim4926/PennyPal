...
import com.ssafy.pennypal.domain.team.dto.response.TeamDetailResponse;
...
//    }
    ...
    void createTeam() {
        // given
        Member member1 = createMember("member1@pennypal.site", "닉1", LocalDate.now());

        Member save = memberRepository.save(member1);

        TeamCreateServiceRequest request = createServiceRequest("팀이름1", true, save.getMemberId(), "팀소개");

        // when
        TeamDetailResponse teamResponse = teamService.createTeam(request);

        // then
        assertThat(teamResponse)
                .extracting("teamIsAutoConfirm", "teamName")
                .contains(true, "팀이름1");
    ...
    void memberCanHaveOneTeam() {
        // given
        Member mem1 = createMember("member1@pennypal.site", "짠1", LocalDate.now());
        Member mem2 = createMember("member2@pennypal.site", "짠2", LocalDate.now());

        memberRepository.saveAll(List.of(mem1, mem2));

        Member member1 = memberRepository.findByMemberEmail("member1@pennypal.site");
        Member member2 = memberRepository.findByMemberEmail("member2@pennypal.site");

        TeamCreateServiceRequest request1 =
                TeamCreateServiceRequest.builder()
                .teamName("팀이름1")
                .teamIsAutoConfirm(true)
                .teamLeaderId(member1.getMemberId())
                .teamInfo("팀소개")
                .build();

        TeamCreateServiceRequest request2 =
                TeamCreateServiceRequest.builder()
                .teamName("팀이름12")
                .teamIsAutoConfirm(true)
                .teamLeaderId(member2.getMemberId())
                .teamInfo("팀소개")
                .build();
        ...
        TeamCreateServiceRequest request3 =
                TeamCreateServiceRequest.builder()
                .teamName("팀이름33243423")
                .teamIsAutoConfirm(true)
                .teamLeaderId(member2.getMemberId())
                .teamInfo("팀소개")
                .build();
    ...
    void teamMembersSizeMustBeLessThan6() {
        ...
        List<Member> savedMembers = memberRepository.saveAll(members.subList(0, 7));

        // 팀 생성, 저장
        Team team = createTeam("팀이름16", true, savedMembers.get(0).getMemberId(), "팀소개1", members.get(0));
        ...
        TeamJoinServiceRequest joinRequest = joinServiceRequest(savedTeam.getTeamId(), savedMembers.get(6).getMemberId());
    ...
    void TeamIsAutoConfirmTrue() {
        ...
        TeamDetailResponse savedTeam = teamService.createTeam(createRequest);
...
//                .containsExactlyInAnyOrder("짠1", "짠2");
    ...
    @DisplayName("팀의 가입 승인 방식이 자동이 아니라면 팀의 가입 대기 리스트에 유저 정보가 들어가고, null을 반환한다.")
    @Test
    void TeamIsAutoConfirmFalse() {
        ...
        Member savedMember1 = memberRepository.findByMemberNickname("짠1");
        Member savedMember2 = memberRepository.findByMemberNickname("짠2");

        TeamCreateServiceRequest createRequest = createServiceRequest("팀이름21", false, savedMember1.getMemberId(), "팀소개");
        TeamDetailResponse savedTeam = teamService.createTeam(createRequest);
        ...
        assertThat(findedTeam.getTeamWaitingList().get(0).getMemberId()).isEqualTo(savedMember2.getMemberId());
        assertThat(savedMember2.getMemberWaitingTeam().getTeamId()).isEqualTo(findedTeam.getTeamId());
...
//        Member member2 = createMember("member2@pennypal.site", "닉네임2", LocalDate.now());
//        Member member3 = createMember("member3@pennypal.site", "닉네임3", LocalDate.now());
//        Member member4 = createMember("member4@pennypal.site", "닉네임4", LocalDate.now());
//        Member member5 = createMember("member5@pennypal.site", "닉네임5", LocalDate.now());
//
//        // 지출 내역 생성
...
//        // given
//        Member member1 = createMember("member1@pennypal.site", "닉네임1", LocalDate.now());
//        Member member2 = createMember("member1@pennypal.site", "닉네임2", LocalDate.now());
//        Member member3 = createMember("member1@pennypal.site", "닉네임3", LocalDate.now());
//        memberRepository.saveAll(List.of(member1,member2,member3));
//
